...
void unpackingBytes( Struct_Pack *message, uint8_t *array )
{
    ...
    /* placing the data in the union structure */
    local_union.local_message = *message;
    /* moving the last 2 bytes to avoid the empty byte */
    ...
    /* copying the result array to the output array */

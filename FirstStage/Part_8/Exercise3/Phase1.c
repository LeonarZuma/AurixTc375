...
static void swap_arrayElements(uint16_t *element_src, uint16_t *element_dst)
{
...
uint8_t arrayCompare( uint8_t *arrayA, uint8_t *arrayB, uint8_t size)
{
    uint8_t local_cmp_result = FALSE;
    uint8_t exit_flag = FALSE;
    uint8_t i = 0;
    while ((i<size) && (exit_flag == FALSE))
    {
        if(*(arrayA + i) == *(arrayB + i))
        {
            local_cmp_result = TRUE;
            exit_flag = FALSE;
            i++;
        }
        else
        {
            local_cmp_result = FALSE;
            exit_flag = TRUE;
        }
    }
    return local_cmp_result;
}
    ...
    for (int8_t cycle = 0; cycle < max_cycle_iter; cycle++)
    {
        ...
        while(local_is_high_flag == FALSE)
        {
            ...
            else
            {
                ...
                if(high_elements>0)
                {
                    swap_arrayElements((array + cycle), (array +cycle+ high_elements));
                    local_is_high_flag = FALSE;
                    high_elements = 0;
                    local_correction_index = cycle + 1;
                }

/*******************************************************************************************
 TO DO: 
 implement the succesful check of the copied data.

 Ask arturo something related to queue buffer
 *******************************************************************************************/
...
uint8_t AppQueue_writeData( AppQue_Queue *queue, void *data )
{
    ...
    {        
        ...
        write_operResult = strncmp((queue->buffer + queue->head), data, queue->size);
    ...
    return ((write_operResult == FALSE)? TRUE:FALSE);
    /* An alternative implementation can be done using bitwise operations. With a toggle
        return (write_operResult ^ 0x01);
    */
}
...
uint8_t AppQueue_readData( AppQue_Queue *queue, void *data )
{
    ...
    if(queue->empty == FALSE)
    {
        ...
        read_operResult = strncmp((queue->buffer + queue->tail), data, queue->size);
    ...
    return ((read_operResult == FALSE)? TRUE:FALSE);
    /* An alternative implementation can be done using bitwise operations. by a toggle
        return (read_operResult ^ 0x01);
    */
}
...
void AppQueue_flushQueue( AppQue_Queue *queue )
{
    ...
    /* Reset the Head and Tail position */

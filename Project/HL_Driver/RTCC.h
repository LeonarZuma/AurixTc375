...
/* MACROS LIKE FUNCTION TO BE SUBSTITE IN CASE OF BEIGN NECESSARY */
/* Software interrupt routine enable for a given source node */
#define SW_IR_EN(src_node)       IfxSrc_enable(&MODULE_SRC.GPSR.GPSR[GPSR0].SR[src_node])
/* Software interrupt routine request for a given source node */
#define SW_IR_RQ(src_node)       IfxSrc_setRequest( &MODULE_SRC.GPSR.GPSR[GPSR0].SR[src_node]);
...
typedef union _AppRtcc_Ctrl
{
  ...
  */
    struct _bits               
    {
        ...
        uint8_t al_int : 1;     /*!< alarm active flag      */
        uint8_t none : 3;       /*!< trigger an interrupt every time the alarm is enable and the hours and minutes match  */

 ...
 *
 * This has a weak attribute to make possible the user redefine this function in their application
 * 
 * @param[in]   None
 *
 * @retval  None
 * 
 * */
__attribute__((weak)) void AppRtcc_callbackAlarm( void )
{
    /*this fucntion shall remian empty, the user application will
    redefine this same fucntion without the weak qualifier and with
    the code to execute according to its application needs*/
}
 ...
 *
 * Enable alarm interrupts only if the alarm is set, to later be triggered by the periodic task only when
 * minutes and hour matches
 *
 * @param[in]   rtcc  RTCC control structure
 * @param[in]   node  parameter is the service request node to use as alarm interrupt source 
 *
 * @retval  None
 * 
 * */
void AppRtcc_intAlarmEnable( AppRtcc_Rtcc *rtcc, uint8_t node )
{
    if(rtcc->ctrl.bits.al_set == TRUE)
    {
        rtcc->ctrl.bits.al_int = TRUE;
        /* saving the source node interrupt for the alarm */
        rtcc->in_src_node = node;
        /* Enable the interrupt for service request node, GPSR0 SR0*/
        SW_IR_EN(node);
    }
    else
    {
